...
echo "🔧 Fixing Office Web (OWA)..."

Office_DOMAIN="office.live.com"
USER_PROFILE="$HOME"
REAL_USER=$(logname)

# 🔹 Clear Chrome cache for Office
clear_chrome() {
    echo "🧹 Clearing Chrome data for $Office_DOMAIN..."
    sqlite3 "$USER_PROFILE/.config/google-chrome/Default/Cookies" \
        "DELETE FROM cookies WHERE host_key LIKE '%$Office_DOMAIN%';" 2>/dev/null
...
# 🔹 Clear Firefox cache for Office
clear_firefox() {
    close_firefox

    echo "🧹 Attempting to clear Firefox data for $Office_DOMAIN..."
    ...
    if [ -n "$profile" ] && [ -d "$profile" ]; then
        ...
        sqlite3 "$profile/cookies.sqlite" \
            "DELETE FROM moz_cookies WHERE host LIKE '%$Office_DOMAIN%';" 2>/dev/null
...
# 🔹 Clear Edge cache for Office
clear_edge() {
    echo "🧹 Clearing Edge data for $Office_DOMAIN..."
...
echo "✅ Office Web cache cleanup completed."
