Write-Host "🔧 Fixing Office Web (OWA)..."

$OfficeDomain = "office.live.com"

# 🔹 Clear Chrome cache for Outlook
function Clear-Chrome {
    Write-Host "🧹 Clearing Chrome data for $OfficeDomain..."
    ...
    if (Test-Path $chromeCookies) {
        try {
            sqlite3 $chromeCookies "DELETE FROM cookies WHERE host_key LIKE '%$OfficeDomain%';" 2>$null
        } catch {
...
function Clear-Firefox {
    ...
    if ($profile) {
        ...
        if (Test-Path $cookiesPath) {
            try {
                sqlite3 $cookiesPath "DELETE FROM moz_cookies WHERE host LIKE '%$OfficeDomain%';" 2>$null
            } catch {
...
function Clear-Edge {
    Write-Host "🧹 Clearing Edge data for $OfficeDomain..."
    ...
    if (Test-Path $edgeCookies) {
        try {
            sqlite3 $edgeCookies "DELETE FROM cookies WHERE host_key LIKE '%$OfficeDomain%';" 2>$null
        } catch {
...
Write-Host "✅ Office Web cache cleanup completed."
